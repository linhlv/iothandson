...
module.exports = function(app, passport, isLoggedIn) {
	...
	router.delete('/publications/:connectionId/:id', isLoggedIn, function(req, res) {		
		req.checkParams('connectionId', 'Connection Id is required').notEmpty();
		req.checkParams('id', 'Id is required').notEmpty();
		req.getValidationResult().then(function(result) {
			if (!result.isEmpty()) {
			...
			Publication.remove({_id: req.params.id}, function (err) {
				if (err)
					res.send(err);

				res.json({ message: 'Publication is removed!' });
			});
		});  		
	});
